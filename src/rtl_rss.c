/*
 * rtl-rss, uses your SoapySDR-driven receiver to compute spectrograms and
 * format them according to the protocol defined here:
...
{
	fprintf(stderr,
		"rx_rss (based on rtl_sdr), an spectrogram server for SoapySDR-supported receivers\n\n"
		"Usage:\t -f frequency_to_tune_to [Hz]\n"
		"\t[-s samplerate (default: 2048000 Hz)]\n"
		"\t[-d device key/value query (ex: 0, 1, driver=rtlsdr, driver=lime)]\n"
		...
		"\turl (a '-' dumps samples to stdout, '@' timestamps only)\n\n");
	exit(1);
}
...
int main(int argc, char **argv)
{
	...
	if(strcmp(url, "-") == 0) {			/* Write samples to stdout */
		file = stdout;
#ifdef _WIN32
		_setmode(_fileno(stdin), _O_BINARY);
#endif
	} else if(strcmp(url, "@") == 0) {	/* do not write any samples, just time the process */
		file = NULL;
	//test for server://x.y.z.w:8888 string to serve spectrogram data
	} else {
		file = fopen(url, "wb");
		if (!file) {
	...
	if (true || sync_mode) {
		...
		while (!do_exit) {
			...
			if (output_format == SOAPY_SDR_CS16) {
				...
				if (file && fwrite(buffer, sizeof(int16_t), n_read, file) != (size_t)n_read) {
				...
				if (file && fwrite(buf8, sizeof(uint8_t), n_read, file) != (size_t)n_read) {
					fprintf(stderr, "Short write, samples lost, exiting!\n");
					break;
				}
				...
				if (file && fwrite(fbuf, sizeof(float), n_read, file) != (size_t)n_read) {
					fprintf(stderr, "Short write, samples lost, exiting!\n");
					break;
				}
			}
			...
			now = clock();
			if ((now - last_report) > CLOCKS_PER_SEC) {
				double elapsed = ((double)now - (double)start) / (double)CLOCKS_PER_SEC;
				double rate = (double)total / elapsed;
				fprintf(stderr,"\r%I64u bytes, %.0lf bytes/sec\r", total, rate);
				last_report = now;
			}
		}
	}
	...
	if (true) {
		double elapsed = ((double)now - (double)start) / (double)CLOCKS_PER_SEC;
		double rate = (double)total / elapsed;
		fprintf(stderr,"\r%I64u bytes, %.0lf bytes/sec\n", total, rate);
	}
